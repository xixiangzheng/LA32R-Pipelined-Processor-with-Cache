...
module cache #(
    ...
    reg     [7:0]               WAY_sel_buf;
    reg     [7:0]               WAY_hit_sel;
    wire    [7:0]               WAY_miss_sel;
    wire                        valid_WAY       [0:WAY_NUM-1];
    wire                        dirty_WAY       [0:WAY_NUM-1];
    wire    [TAG_WIDTH-1:0]     r_tag_WAY       [0:WAY_NUM-1];
    wire    [LINE_WIDTH-1:0]    r_line_WAY      [0:WAY_NUM-1];
    wire    [WAY_NUM-1:0]       tag_we_WAY;
    wire    [WAY_NUM-1:0]       data_we_WAY;
    
    // “路”的选择
    assign WAY_sel = hit ? WAY_hit_sel : WAY_miss_sel;
    assign valid = valid_WAY[WAY_sel];
    assign dirty = dirty_WAY[WAY_sel];
    assign r_tag = r_tag_WAY[WAY_sel];
    assign r_line = r_line_WAY[WAY_sel];
    assign tag_we_WAY = refill ? tag_we << WAY_sel_buf : tag_we << WAY_sel;
    assign data_we_WAY = refill ? data_we << WAY_sel_buf : data_we << WAY_sel;
    ...
    generate
        ...
        begin:  get_from_bram
            ...
            bram #(
                ...
                .we(tag_we_WAY[i]),
                .dout({valid_WAY[i], dirty_WAY[i], r_tag_WAY[i]})
            );
    ...
    always @(*) begin
        ...
        for (k = 0; k < WAY_NUM; k = k + 1) begin
            if (valid_WAY[k] && (r_tag_WAY[k] == tag)) begin
                hit = 1;
                WAY_hit_sel = k;
            end
        end
